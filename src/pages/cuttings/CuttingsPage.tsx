...
export function CuttingsPage() {
  ...
  // "Maturing" = still working toward the estimated ready date: callusing in the
  // nursery, sourced (not yet grafted), or grafted-and-rooting. Includes new
  // records that start life in 'In Nursery / Callusing' or 'Sourced'.
  ...
  const columns: Column<CuttingBatch>[] = [
    ...
    },
    {
      ...
      accessor: (b) => {
        // Planted batches are deployed in the field and feed the wholesale forecast.
        if (b.planted) {
          return (
        ...
        // Rooted-ready internal batches: the staff packing action lives here.
        if (b.status === CUTTING_STATUS_ROOTED_READY && b.source !== CUTTING_SOURCE_CUSTOMER) {
          const replanted = b.allocation === CUTTING_ALLOCATION_REPLANT;
          return (
            <div className="flex flex-col items-start gap-1.5">
              {replanted ? (
                // Flagged for the farm → deploy into the field.
                <Button
                  size="xs"
                  variant="outline"
                  icon={<Sprout className="w-3.5 h-3.5" />}
                  onClick={() => markPlanted(b.id)}
                >
                  Mark as Planted
                </Button>
              ) : (
                // Default → pack for delivery (releases to Available Stock for Sale).
                <Button
                  size="xs"
                  icon={<PackageCheck className="w-3.5 h-3.5" />}
                  onClick={() => markPacked(b.id)}
                >
                  Mark as Packed
                </Button>
              )}
              {/* Toggle the destination: keep for the farm (replant) vs. pack for sale. */}
              <button
                type="button"
                onClick={() => allocateBatch(b.id, replanted ? '' : CUTTING_ALLOCATION_REPLANT)}
                className={[
                  'text-xs px-1.5 py-0.5 rounded transition-colors',
                  replanted ? 'bg-purple-100 text-purple-700' : 'text-gray-500 hover:text-purple-700 hover:bg-purple-50',
                ].join(' ')}
                aria-pressed={replanted}
              >
                {replanted ? '↩ Flagged for Replant' : 'For Replant in Farm'}
              </button>
            </div>
          );
        }
    ...
    <div className="space-y-6">
      ...
      {rootedReady.length > 0 && (
        <SectionCard
          ...
          subtitle='These batches have finished rooting — click "Mark as Packed" on each to release it into Available Stock for Sale'
        >
          <div className="flex flex-wrap gap-2">
            {rootedReady.map((b) => (
              <div key={b.id} className="flex items-center gap-2 px-3 py-1.5 bg-green-50 border border-green-300 rounded-lg">
                <span className="text-sm font-medium text-green-800">{b.subcategory}</span>
                <Badge label={`${formatNumber(b.quantityAvailable, 0)} ready`} variant="green" />
                {b.allocation === CUTTING_ALLOCATION_REPLANT ? (
                  <Badge label="For Replant" variant="purple" />
                ) : (
                  <Button size="xs" icon={<PackageCheck className="w-3.5 h-3.5" />} onClick={() => markPacked(b.id)}>
                    Mark as Packed
                  </Button>
                )}
              </div>
      ...
      {rooting.length > 0 && (
        <SectionCard title="⏳ Rooting" subtitle="Cuttings still maturing — callusing, sourced, or rooting toward their estimated ready date">
          <div className="flex flex-wrap gap-2">
            {rooting.map((b) => (
