    ...
    <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 space-y-2">
      ...
      <div className="space-y-1.5">
        {matches.map((m) => (
          <div
            key={keyOf(m.item)}
            className="flex items-center justify-between gap-2 rounded-md bg-white border border-amber-100 px-2.5 py-1.5"
          >
            <div className="min-w-0">
              <p className="text-sm font-medium text-gray-800 truncate">{labelOf(m.item)}</p>
              <p className="text-xs text-amber-600">matches on {m.matchedOn.join(', ')}</p>
            </div>
            {onUseExisting && (
              <button
                type="button"
                onClick={() => onUseExisting(m.item)}
                className="flex-shrink-0 px-2.5 py-1 text-xs font-medium bg-white border border-amber-300 rounded-full text-amber-700 hover:bg-amber-100"
              >
                Open existing
              </button>
            )}
